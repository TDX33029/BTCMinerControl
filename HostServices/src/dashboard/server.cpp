...
.board-table{width:100%;min-width:1050px;border-collapse:collapse;table-layout:auto;font-size:12px}
.board-table th,.board-table td{border:1px solid #a7a7a7;padding:4px 6px;text-align:left;white-space:nowrap;height:27px}
...
.latency-value{display:inline-block;min-width:64px;font-family:'Courier New',monospace}
...
<div class="workspace">
  <div class="board-table-wrap">
    <table class="board-table">
      <thead><tr>
        <th>Board ID</th><th>IP</th><th>Status</th><th>Latency (ms)</th><th>Test</th><th>ASIC</th>
        <th>Hashrate</th><th>Power (W)</th><th>Temperature (°C)</th>
        <th>TPS Power</th><th>Jobs</th><th>Nonces</th>
        <th>Shares A/R</th><th>Best Diff</th>
      </tr></thead>
...
function updateUI(data) {
    ...
    const boardTableHtml = data.boards.map(b => {
        ...
        return `<tr class="${b.online?'online':'offline'}">
            <td><strong>${boardId}</strong></td><td>${escapeHtml(b.ip_addr||'-')}</td><td>${online}</td><td class="latency-cell">${latencyHtml}</td><td>${testState}</td>
            <td class="number">${b.asic_count}</td><td class="number">${formatHashrate(b.hashrate_1m)}</td>
            <td class="number">${power}</td><td class="number">${temperature}</td>
...
void DashboardServer::handleClient(SOCKET client) {
    ...
    if (version.rfind("HTTP/", 0) != 0) {
        ...
        uint64_t board_id = 0;
        if (!has_control_header(request) ||
            !parse_board_id_query(query_string, board_id)) {
            response = http_response(400, "Bad Request",
        ...
        if (!has_control_header(request) || !m_boards) {
            response = http_response(400, "Bad Request",
                "application/json; charset=utf-8", "{\"ok\":false}");
        } else {
        ...
        bool enabled = false;
        if (!has_control_header(request) ||
            !parse_power_query(query_string, board_id, enabled)) {
            response = http_response(400, "Bad Request",
                "application/json; charset=utf-8", "{\"ok\":false}");
