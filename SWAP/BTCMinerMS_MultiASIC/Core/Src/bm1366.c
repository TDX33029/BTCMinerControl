...
void bm1366_send_job(const struct protocol_job_t *job) {
    /* Build the BM1366 work packet data (big-endian on the wire, matching
       ...
       Note: HostServices already performs word-reversal on merkle_root and prev_block_hash. */
    static uint32_t jobs_sent_diag = 0;
    ...
    if (jobs_sent_diag < 2) {
        printf("[JOB-TX] #%lu id=%u len=%u ver=0x%08lX nbits=0x%08lX\r\n",
               (unsigned long)jobs_sent_diag, (unsigned int)job->job_id, (unsigned int)pos,
               (unsigned long)job->version, (unsigned long)job->nbits);
        jobs_sent_diag++;
    }
...
void bm1366_read_chip_register(uint8_t asic_nr, uint8_t reg) {
    uint8_t interval = (bm1366_address_interval > 0) ? bm1366_address_interval : (256 / ((bm1366_chip_count > 0) ? bm1366_chip_count : 1));
    uint8_t addr = asic_nr * interval;
    uint8_t data[] = {addr, reg};
    bm1366_send_cmd(BM1366_TYPE_CMD | BM1366_GROUP_SINGLE | BM1366_CMD_READ, data, 2);
}
...
void bm1366_set_nonce_space(double nonce_percent, float frequency,
    ...
    double hcn_max  = (double)hcn_space * (double)BM1366_FREQ_MULT / (double)frequency * 0.015625;  /* 0.015625 = 1/64, HCN ~= Bitaxe's 105K */
 ...
 int bm1366_init_chips(uint8_t expected_count, float target_freq_mhz) {
     ...
     bm1366_set_difficulty_mask(BM1366_DEFAULT_DIFFICULTY);
     ...
     /* 平滑阶梯升频至目标工作频率 (6.25MHz/步，确保 PLL 稳态锁定) */
     bm1366_frequency_transition(target_freq_mhz, 30);
     float actual_freq = (float)bm1366_get_actual_frequency();
     uint16_t cores = BM1366_CORES;
     bm1366_set_nonce_space(1.0, actual_freq, chips, cores);
     printf("[FREQ] BM1366 target=%u MHz, actual=%.2f MHz, HCN space partitioned for %u chip(s)\r\n",
            (unsigned int)bm1366_target_freq_mhz, actual_freq, (unsigned int)chips);
...
uint16_t bm1366_get_frame_errors(void) { return uart_rx_frame_errors; }
uint16_t bm1366_get_overruns(void) { return uart_rx_overruns; }
...
void bm1366_frequency_transition(float target_freq_mhz, uint32_t step_delay_ms) {
    float current_freq = (float)bm1366_actual_freq_mhz;
    if (current_freq < 50.0f) {
        current_freq = 50.0f;
        bm1366_set_frequency(50.0f);
        HAL_Delay(step_delay_ms);
    }
    float diff = fabsf(target_freq_mhz - current_freq);
    if (diff < FREQ_EPSILON) {
        float actual = bm1366_set_frequency(target_freq_mhz);
        bm1366_target_freq_mhz = (uint16_t)target_freq_mhz;
        bm1366_actual_freq_mhz = (uint16_t)(actual + 0.5f);
        return;
    }
    if (diff < FREQ_STEP_SIZE) {
        float actual = bm1366_set_frequency(target_freq_mhz);
        bm1366_target_freq_mhz = (uint16_t)target_freq_mhz;
        bm1366_actual_freq_mhz = (uint16_t)(actual + 0.5f);
        return;
