 ...
 int protocol_decode_version_mask(const uint8_t *data, uint16_t len,
                                  protocol_version_mask_t *mask) {
     if (len < 4) return 0;
     mask->version_mask = read_u32(data);
     return 1;
 }

 int protocol_decode_set_frequency(const uint8_t *data, uint16_t len,
                                   protocol_set_frequency_t *freq) {
     if (len < 2) return 0;
     freq->frequency_mhz = ((uint16_t)data[0] << 8) | data[1];
     return 1;
 }
 ...
 uint16_t protocol_encode_hello(const protocol_hello_t *hello, uint8_t *buf) {
     ...
     write_u16(payload + 12, hello->target_frequency_mhz);
     write_u16(payload + 14, hello->actual_frequency_mhz);
     return build_frame(MSG_BOARD_HELLO, payload, sizeof(payload), buf);
 }
 
 uint16_t protocol_encode_hashrate(const protocol_hashrate_t *hashrate,
                                    uint8_t *buf) {
     uint8_t payload[4];
     write_u32(payload, hashrate->hashrate_mhs);
     return build_frame(MSG_HASHRATE, payload, sizeof(payload), buf);
 }

 uint16_t protocol_encode_frequency_status(const protocol_frequency_status_t *freq,
                                          uint8_t *buf) {
     uint8_t payload[4];
     write_u16(payload, freq->target_mhz);
     write_u16(payload + 2, freq->actual_mhz);
     return build_frame(MSG_FREQUENCY_STATUS, payload, sizeof(payload), buf);
 }
