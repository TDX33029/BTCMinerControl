...
#define BM1366_EXPECTED_COUNT  6    /* 最大支持 6 颗 ASIC 链式探测与初始化 */
#define BM1366_TARGET_FREQ_MHZ 100.0f  /* 默认 ASIC 基础安全工作频率 (200MHz) */
/* TPS546D24A 已弃用：供电已更换为独立免配置电源。
   相关 PMBus 配置宏已关闭，保留 TMP1075 板载温度检测。 */
#define TPS546_ENABLED             0
...
/* 开启终端串口 Nonce 结果实时打印输出 */
#define BM1366_LOG_RESULTS    1
...
int main(void)
{
  ...
  while (1) {
    ...
    /* Periodic ASIC health poll: alternate reading Chip ID (0x00) and Total Hash Count (0x8C)
       from each chip individually so they never collide on the return path. */
    static uint32_t last_chip_poll = 0;
    if (asic_ready && (now - last_chip_poll) >= 5000U) {
        last_chip_poll = now;
        static uint8_t poll_phase = 0;
        uint8_t count = bm1366_get_chip_count();
        if (count > 0) {
            uint8_t chip_idx = (poll_phase / 2) % count;
            uint8_t reg = (poll_phase % 2 == 0) ? 0x8C : 0x00;
            bm1366_read_chip_register(chip_idx, reg);
            poll_phase++;
        }
    }
  ...
  if (connected && (now - last_hello) > 20000) {   /* 20s keepalive hello (was 60s -- server closed on idle) */
    ...
    if ((now - last_status) > 10000) {
        ...
        last_status = now;
        eth_get_ip(ip);
        printf("\r\n[STATUS] up=%lus  ip=%u.%u.%u.%u  eth=%s  bm1366=%s  chips=%d  link=%s\r\n"
               "         ETH: rx=%lu tx=%lu | UART1: rx_total=%lu bad_pre=%lu bad_crc=%lu FE=%u ORE=%u\r\n",
               ...
               (unsigned long)bm1366_get_rx_total(),
               (unsigned long)bm1366_get_bad_preamble(),
               (unsigned long)bm1366_get_bad_crc(),
               (unsigned int)bm1366_get_frame_errors(),
               (unsigned int)bm1366_get_overruns());
...
static void apply_board_frequency(uint16_t target_mhz) {
    /* 暂时忽略上位机下发的频率调节命令，固定在安全可靠的基础频率 (200MHz)，
       防止多芯片在瞬间切到 485MHz 造成过载或失锁。向 host 如实反馈当前频率。 */
    printf("[FREQ] host request %u MHz -> IGNORED (locked at %u MHz for stability)\r\n",
           (unsigned int)target_mhz, (unsigned int)bm1366_get_actual_frequency());
...
static void check_bm1366_results(void) {
    ...
    while (bm1366_read_any_result(&raw, 0) > 0) {
        ...
        printf("[RAWFRAME] %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X (flag=0x%02X)\r\n",
               rp[0], rp[1], rp[2], rp[3], rp[4], rp[5], rp[6], rp[7], rp[8], rp[9], rp[10], raw.crc_and_flags);

        if (raw.crc_and_flags & 0x80U) {
            /* 结果包解析:
               - parsed.nonce: 保持原始 wire 字节序 (小端 host 值)，上位机以 LE 直接拼入 80B 区块头第 76..79 字节
               - nonce_h: raw.nonce 即为小端主机值，MSB (bit 25..31) 包含 Core ID，bit 17..24 包含 ASIC 地址 */
            bm1366_result_t parsed = {0};
            uint32_t nonce_h = raw.nonce;
            ...
            uint8_t interval    = bm1366_get_address_interval();
            parsed.asic_nr      = (interval > 0) ? (((nonce_h >> 17) & 0xFF) / interval) : 0;
            {
            ...
            printf("[NONCE] job_id=%d nonce=0x%08X (raw=0x%08X) core=%d small=%d asic=%d rolled_ver=0x%08X\r\n",
                   parsed.job_id, nonce_h, parsed.nonce, parsed.core_id, parsed.small_core_id, parsed.asic_nr, parsed.rolled_version);

            uint8_t buf[64];
            uint16_t len = protocol_encode_nonce(&parsed, BOARD_ID, buf);
            if (len > 0) {
                eth_send(buf, len);
                printf("[NET-NONCE] Forwarded %u bytes to Host\r\n", (unsigned int)len);
            }
            continue;
        } else {
            /* 寄存器读取回包解析:
               raw.midstate_num = asic_address (0x00, 0x80, ...)
               raw.job_id_raw   = register_address (0x00, 0x8C, ...)
               raw.nonce        = 32-bit register value */
            uint8_t asic_addr = raw.midstate_num;
            uint8_t reg_addr  = raw.job_id_raw;
            uint32_t val      = __REV(raw.nonce);
            uint8_t interval  = bm1366_get_address_interval();
            uint8_t asic_nr   = (interval > 0) ? (asic_addr / interval) : 0;
            printf("[CHIP-REG] asic=%u (addr=0x%02X) reg=0x%02X val=0x%08lX (%lu)\r\n",
                   (unsigned int)asic_nr, (unsigned int)asic_addr, (unsigned int)reg_addr,
