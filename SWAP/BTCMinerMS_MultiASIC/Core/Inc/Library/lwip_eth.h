...
#include "stm32f1xx_hal.h"   /* HAL provides ETH register defs */
#include <stdint.h>

/* Force 10M Full-Duplex (workaround for 100M RMII RX signal-integrity issues
   that cause CRC + dribble-bit errors on every received frame). When 1, the
   PHY advertises only 10M (ANAR) and the MAC uses FES=0. Purely register
   writes -- does NOT change any pins or wiring. Flip to 0 for 100M. */
#define ETH_FORCE_10M 1
...
#define ETH_MACFFR_HPF           ((uint32_t)0x00000400U)
#endif
...
#define ETH_DMABMR_MB            ((uint32_t)0x00000080U)
#endif
...
#define ETH_DMAOMR_DTCEFD        ((uint32_t)0x04000000U)
#endif
...
#define ETH_DMAOMR_SR            ((uint32_t)0x00000002U)
#endif
#ifndef ETH_DMASR_TPS
#define ETH_DMASR_TPS            ((uint32_t)0x00700000U)
#endif
