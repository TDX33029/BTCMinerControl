...
/* Mix the boot tick with the factory UID so boards that power up together do
   not all start lwIP with the same predictable initial sequence number. */
#define LWIP_RAND()                     ((u32_t)(HAL_GetTick() ^ (*(volatile uint32_t *)0x1FFFF7E8U)))
