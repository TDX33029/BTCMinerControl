 ...
 #define BM1366_DEFAULT_FREQ  200.0f
 #define BM1366_DEFAULT_DIFFICULTY 1   /* 默认难度掩码: 1 (每个 2^32 空间稳定产出 Nonce 用于联调) */
...
int  bm1366_init_chips(uint8_t expected_count, float target_freq_mhz);
 uint32_t bm1366_get_active_baud(void);
...
uint16_t bm1366_get_frame_errors(void);
uint16_t bm1366_get_overruns(void);
