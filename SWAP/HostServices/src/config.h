...
struct AppConfig {
    ...
    std::string dashboard_password;         // plaintext fallback; actions disabled when empty
    std::string dashboard_password_sha256; // hex double-SHA256(salt_bytes || password)
    std::string dashboard_password_salt;   // hex salt (preferred over plaintext)
    bool frequency_control_enabled = false; // temporary disable switch for ASIC frequency UI
