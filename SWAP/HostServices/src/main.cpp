...
void update_dashboard(DashboardServer& dashboard, BoardManager& boards,
    ...
    for (const auto& board : boards.getStats()) {
        if (board.online) total_hashrate += board.info.current_hashrate;
    }
...
int main(int argc, char* argv[]) {
        ...
        if (argument == "--uart-test") uart_test_mode = true;
            ...
            if (i + 1 >= argc) {
                std::cerr << "Usage: " << argv[0]
                          << " --hash-password <password>" << std::endl;
                return 2;
            }
            const std::string password = argv[++i];
            uint8_t salt[16]{};
            const uint64_t salt_a = platform::secure_random_u64();
            const uint64_t salt_b = platform::secure_random_u64();
            std::memcpy(salt, &salt_a, sizeof(salt_a));
            std::memcpy(salt + 8, &salt_b, sizeof(salt_b));
            std::vector<uint8_t> input;
            input.reserve(sizeof(salt) + password.size());
            input.insert(input.end(), std::begin(salt), std::end(salt));
            input.insert(input.end(), password.begin(), password.end());
            const auto digest =
                sha256::double_sha256(input.data(), input.size());
            std::cout << "Add these fields to config.json (replace the "
                         "plaintext dashboard_password):\n";
            std::cout << "  \"dashboard_password_sha256\": \""
                      << bin2hex(digest) << "\",\n";
            std::cout << "  \"dashboard_password_salt\": \""
                      << bin2hex(salt, sizeof(salt)) << "\"\n";
            return 0;
        }
    ...
    if (!dashboard.start(config.dashboard_port, &board_manager,
                         ...
                         config.dashboard_password_sha256,
                         config.dashboard_password_salt,
                         config.frequency_control_enabled)) {
    ...
    board_manager.onBoardConnected = [&](uint64_t board_id) {
        ...
        if (config.frequency_control_enabled) {
            ...
            std::cout << "[main] Frequency control disabled; board 0x"
                      << std::hex << board_id << std::dec
                      << " keeps its firmware default frequency" << std::endl;
        }
