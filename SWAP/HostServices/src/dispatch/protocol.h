...
//   0x0B Board→PC: Measured hashrate sample (MH/s)
//   0x0C PC→Board: Set ASIC frequency (MHz)
//   0x0D Board→PC: ASIC frequency status
...
// Decode a measured hashrate sample (megahash/s fixed point).
bool decode_hashrate_sample(const uint8_t* data, size_t len, uint32_t& mhs);

// Encode a frequency-only adjustment command (MHz).
std::vector<uint8_t> encode_set_frequency(uint16_t frequency_mhz);

// Decode the board's applied/actual frequency status.
