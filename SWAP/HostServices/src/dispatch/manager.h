...
struct BoardStats {
    ...
    uint64_t hashrate_updated = 0;
    uint64_t last_share_accepted_ms = 0;
...
public:
    ...
    bool setBoardFrequency(uint64_t board_id, uint16_t frequency_mhz);
...
private:
    ...
    void processHashrateSample(uint64_t board_id, uint32_t hashrate_mhs);
    ...
    std::unordered_map<uint64_t,
        std::deque<std::pair<uint64_t, double>>> m_hashrate_samples;
