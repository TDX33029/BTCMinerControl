...
SOCKET connect_test_board(uint16_t port, uint64_t board_id) {
    ...
    uint8_t hello[21] = {0, 0, 0, 17, 0x04};
    for (int i = 0; i < 8; ++i) {
    ...
    hello[15] = 2;
    hello[16] = 0;
    hello[17] = 0x00; hello[18] = 0xC8; // target frequency: 200 MHz
    hello[19] = 0x00; hello[20] = 0xC8; // actual frequency: 200 MHz
    if (::send(socket_handle, reinterpret_cast<const char*>(hello), 3, 0) != 3 ||
        ::send(socket_handle, reinterpret_cast<const char*>(hello + 3), 18, 0) != 18) {
        closesocket(socket_handle);
        return INVALID_SOCKET;
    }
...
bool fetch_dashboard_stats(uint16_t port, nlohmann::json& stats,
                            const std::string& session_cookie = std::string()) {
    SOCKET client = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    ...
    const std::string request =
        ...
        (session_cookie.empty()
             ? std::string()
             : "Cookie: btc_session=" + session_cookie + "\r\n") +
        "Connection: close\r\n\r\n";
...
bool dashboard_login(uint16_t port, const std::string& password,
    ...
    platform::set_recv_timeout(client, 2000);
    const std::string body = "password=" + password;
    const std::string request =
    ...
    closesocket(client);
    if (!ok || response.rfind("HTTP/1.1 200", 0) != 0) return false;

    const std::string marker = "\r\nSet-Cookie: btc_session=";
    const size_t start = response.find(marker);
    if (start == std::string::npos) return false;
    size_t value_start = start + marker.size();
    size_t value_end = response.find(';', value_start);
    if (value_end == std::string::npos) return false;
    session_cookie = response.substr(value_start, value_end - value_start);
...
bool test_chip_test_work_rotation() {
    ...
    if (!dashboard.start(dashboard_port, &boards, "127.0.0.1", "",
                          0, 5000, "test-password")) {
        boards.stop();
        return false;
    }
    ...
    // Firmware reports measured hashrate in MH/s fixed point. Verify the
    // manager turns the sample into a GH/s board/aggregate value.
    std::vector<uint8_t> hashrate_frame = {
        static_cast<uint8_t>(MsgType::HashrateSample),
        0x00, 0x02, 0x00, 0x00  // 131072 MH/s = 131.072 GH/s
    };
    if (ok) ok &= send_test_frame(board, hashrate_frame);
    const auto hashrate_deadline = std::chrono::steady_clock::now() +
                                   std::chrono::seconds(2);
    while (ok && !boards.getStats().empty() &&
           boards.getStats()[0].hashrate_updated == 0 &&
           std::chrono::steady_clock::now() < hashrate_deadline) {
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
    if (ok && !boards.getStats().empty()) {
        const auto board_with_hashrate = boards.getStats()[0];
        ok &= std::abs(board_with_hashrate.info.current_hashrate -
                       131.072) < 0.001;
        ok &= std::abs(board_with_hashrate.hashrate_1m - 131.072) < 0.001;
    }

    std::string session_cookie;
    if (ok) ok &= dashboard_login(dashboard_port, "test-password", session_cookie);

    std::vector<uint8_t> power_frame;
    if (ok) ok &= post_dashboard_power(dashboard_port, board_id, false, session_cookie);
    ...
    latency_frame.clear();
    if (ok) ok &= post_dashboard_action(dashboard_port, "/api/board-latency-all", session_cookie);
    if (ok) ok &= receive_test_frame(board, latency_frame);
    if (ok) ok &= latency_frame.size() == 9 &&
                  latency_frame[0] == static_cast<uint8_t>(MsgType::LatencyProbe);
    if (ok) ok &= send_test_frame(board, latency_frame);
    ...
    boards.addAcceptedShare(board_id);
    bool auth_challenge = false;
    // Anonymous visitors get the login page for page routes and 401 for APIs.
    ok &= dashboard_get_status(dashboard_port, "/", auth_challenge) == 200;
    ok &= !auth_challenge;
    ok &= dashboard_get_status(dashboard_port, "/account", auth_challenge) == 401;
    ok &= !auth_challenge;
    // Authenticated session can access the dashboard and receives normal 404s.
    ok &= dashboard_get_status(dashboard_port, "/account", auth_challenge,
                               session_cookie) == 404;
    ok &= !auth_challenge;
    nlohmann::json stats;
    ok &= fetch_dashboard_stats(dashboard_port, stats, session_cookie);
    if (ok) {
        ...
        if (stats.contains("boards") && stats["boards"].size() == 1) {
            ...
            ok &= board_stats.value("target_frequency_mhz", 0) == 200;
            ok &= board_stats.value("actual_frequency_mhz", 0) == 200;
            ok &= board_stats.value("jobs_sent", 0) == 2;
            ok &= board_stats.value("nonces_returned", 0) == 1;
            ok &= board_stats.value("best_diff", 0.0) == 300.0;
            ok &= board_stats.value("shares_accepted", 0) == 1;
            ok &= board_stats.value("last_share_accepted_ms", 0) > 0;
            ok &= board_stats.value("last_share_accepted_age_seconds", -1.0) >= 0.0;
...
bool run_self_tests() {
    ...
    const auto frequency_frame = encode_set_frequency(500);
    ok &= check(frequency_frame.size() == 7 &&
                frequency_frame[3] == 3 &&
                frequency_frame[4] == 0x0C &&
                frequency_frame[5] == 0x01 &&
                frequency_frame[6] == 0xF4,
                "board frequency command frame");
    uint16_t decoded_target_mhz = 0;
    uint16_t decoded_actual_mhz = 0;
    const uint8_t frequency_status_payload[4] = {0x01, 0xF4, 0x01, 0xF2};
    ok &= check(decode_frequency_status(frequency_status_payload,
                                        sizeof(frequency_status_payload),
                                        decoded_target_mhz,
                                        decoded_actual_mhz) &&
                decoded_target_mhz == 500 && decoded_actual_mhz == 498,
                "board frequency status decode");

    uint32_t decoded_mhs = 0;
    const uint8_t hashrate_payload[4] = {0x00, 0x02, 0x00, 0x00};
    ok &= check(decode_hashrate_sample(hashrate_payload,
                                       sizeof(hashrate_payload),
                                       decoded_mhs) &&
                decoded_mhs == 131072U,
                "board measured hashrate sample decode");
    ...
    ok &= check(test_chip_test_work_rotation(),
                "password-protected dashboard and rotating chip-test jobs");
