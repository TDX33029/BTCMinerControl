...
#define BM1366_EXPECTED_COUNT  2    /* bench: only 1 chip populated */
#define BM1366_TARGET_FREQ_MHZ 200.0f  /* default ASIC working frequency at boot */
/* Core rail setpoint applied to the TPS546D24A at boot (Bitaxe MAX/GAMMA
   uses 1.2 V for BM1366). Also verify TPS_BOARD_SCALE_LOOP in
   tps546d24a.c matches the PCB feedback network. */
#define TPS546_APPLY_BOARD_CONFIG  1
#define BM1366_CORE_VOLTAGE_MV     1200U
...
/* 115200-baud result logging can throttle the main loop, so it is off for
   normal mining. Flip to 1 only while debugging the chip link. */
...
static protocol_job_t last_job;          /* re-sent after an ASIC frequency change */
...
static void tps_report_fault_detail(uint16_t status_word);
...
int main(void)
{
     ...
     to PHY/MAC -> MX_ETH_Init hangs on the DMA SWR (link stays DOWN too). */
  {
    ...
    uint32_t mco = (RCC->CFGR >> 24) & 0x0FU;
    printf("[CLK] PLL3(PLLI2S)ready=%d  RCC_CR=0x%08lX  RCC_CFGR=0x%08lX\r\n",
           p3, (unsigned long)RCC->CR, (unsigned long)RCC->CFGR);
    printf("[CLK] MCO[27:24]=0x%X (0xD=PLL3CLK 50MHz)  CFGR2=0x%08lX "
           "(PREDIV2[7:4]=0x%lX expect 4=div5, PLL3MUL[15:12]=0x%lX expect 8=x10)\r\n",
           (unsigned int)mco, (unsigned long)RCC->CFGR2,
           (unsigned long)((RCC->CFGR2 >> 4) & 0xFU),
           (unsigned long)((RCC->CFGR2 >> 12) & 0xFU));
  }
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  /* Wait for PLL3 lock (feeds MCO/PA8 -> PHY X1 + REF_CLK). Without this,
     MCO can output an unlocked-PLL garbage clock right when MX_ETH_Init
     starts. */
  {
    uint32_t timeout = 0xFFFFU;
    while (!__HAL_RCC_GET_FLAG(RCC_FLAG_PLLI2SRDY) && --timeout);
    if (timeout == 0U) {
      printf("[CLK] ERROR: PLL3 did not lock; MCO will not output 50MHz\r\n");
    }
  }
}
...
static void MX_GPIO_Init(void)
{
  ...
  /* MCO outputs 50 MHz (PLL3): must use the 50MHz GPIO speed setting.
     LOW (2MHz-class slew) collapses the 50MHz waveform -- PA8 then shows
     garbage (was measured ~50kHz) and the PHY/MAC get no usable clock. */
...
static void MX_GPIO_Init_Ext(void)
{
    ...
    /* PA8 MCO (50MHz) - re-assert 50MHz speed in case CubeMX regeneration
       restores the LOW-speed setting in MX_GPIO_Init. */
    GPIO_InitStruct.Pin = GPIO_PIN_8;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
...
static void send_frequency_status(void) {
    protocol_frequency_status_t freq;
    freq.target_mhz = bm1366_get_target_frequency();
    freq.actual_mhz = bm1366_get_actual_frequency();
    uint8_t buf[16];
    uint16_t len = protocol_encode_frequency_status(&freq, buf);
    if (len > 0) eth_send(buf, len);
}

static void apply_board_frequency(uint16_t target_mhz) {
    if (asic_ready) {
        uint16_t actual = bm1366_apply_frequency(target_mhz);
        printf("[FREQ] requested %u MHz -> actual %u MHz\r\n",
               (unsigned int)target_mhz, (unsigned int)actual);
        if (last_job_valid) bm1366_send_job(&last_job);
    } else {
        printf("[FREQ] request %u MHz ignored (no ASIC ready)\r\n",
...
static void send_board_hello(void) {
    protocol_hello_t hello = {
        ...
        .status = 0,
        .target_frequency_mhz = bm1366_get_target_frequency(),
        .actual_frequency_mhz = bm1366_get_actual_frequency()
    };
    uint8_t buf[64];
...
static void tps_report_fault_detail(uint16_t status_word) {
    uint8_t vout = 0, iout = 0, vin = 0, temp = 0, cml = 0, mfr = 0;
    uint8_t have_vout = tps546d24a_read_byte_cmd(&tps546d24a, 0x7AU, &vout);
    uint8_t have_iout = tps546d24a_read_byte_cmd(&tps546d24a, 0x7BU, &iout);
    uint8_t have_vin  = tps546d24a_read_byte_cmd(&tps546d24a, 0x7CU, &vin);
    uint8_t have_temp = tps546d24a_read_byte_cmd(&tps546d24a, 0x7DU, &temp);
    uint8_t have_cml  = tps546d24a_read_byte_cmd(&tps546d24a, 0x7EU, &cml);
    uint8_t have_mfr  = tps546d24a_read_byte_cmd(&tps546d24a, 0x80U, &mfr);

    /* TPS546D24A STATUS_WORD uses the TI-specific layout (not standard PMBus
       bit order below bit 12) - verified against the Bitaxe ESP-Miner
       reference driver (main/power/TPS546.h). */
    printf("[TPS] STATUS_WORD=0x%04X:", (unsigned int)status_word);
    if (status_word & 0x8000U) printf(" VOUT");
    if (status_word & 0x4000U) printf(" IOUT/POUT");
    if (status_word & 0x2000U) printf(" INPUT");
    if (status_word & 0x1000U) printf(" MFR");
    if (status_word & 0x0800U) printf(" PGOOD-not-in-regulation");
    if (status_word & 0x0200U) printf(" OTHER");
    if (status_word & 0x0080U) printf(" BUSY");
    if (status_word & 0x0040U) printf(" OFF");
    if (status_word & 0x0020U) printf(" VOUT_OV");
    if (status_word & 0x0010U) printf(" IOUT_OC");
    if (status_word & 0x0008U) printf(" VIN_UV");
    if (status_word & 0x0004U) printf(" TEMPERATURE");
    if (status_word & 0x0002U) printf(" CML");
    if (status_word & 0x0001U) printf(" VENDOR(b0)");
    printf("\r\n");

    if (have_vout && vout) {
        printf("[TPS] STATUS_VOUT=0x%02X:", (unsigned int)vout);
        if (vout & 0x80U) printf(" VOUT_OV_FAULT");
        if (vout & 0x40U) printf(" VOUT_OV_WARN");
        if (vout & 0x20U) printf(" VOUT_UV_WARN");
        if (vout & 0x10U) printf(" VOUT_UV_FAULT");
        if (vout & 0x08U) printf(" VOUT_MIN_MAX");
        if (vout & 0x04U) printf(" TON_MAX");
        printf("\r\n");
    }
    if (have_iout && iout) {
        printf("[TPS] STATUS_IOUT=0x%02X:", (unsigned int)iout);
        if (iout & 0x80U) printf(" OC_FAULT");
        if (iout & 0x20U) printf(" OC_WARN");
        if (iout & 0x10U) printf(" VENDOR(b4)");
        printf("\r\n");
    }
    if (have_vin && vin) {
        printf("[TPS] STATUS_INPUT=0x%02X:", (unsigned int)vin);
        if (vin & 0x80U) printf(" VIN_OV_FAULT");
        if (vin & 0x20U) printf(" VIN_UV_WARN");
        if (vin & 0x08U) printf(" LOW_VIN(live)");
        printf("\r\n");
    }
    if (have_temp && temp) {
        printf("[TPS] STATUS_TEMPERATURE=0x%02X:", (unsigned int)temp);
        if (temp & 0x80U) printf(" OT_FAULT");
        if (temp & 0x40U) printf(" OT_WARN");
        printf("\r\n");
    }
    if (have_mfr && mfr) {
        printf("[TPS] STATUS_MFR_SPECIFIC=0x%02X:", (unsigned int)mfr);
        if (mfr & 0x80U) printf(" POR");
        if (mfr & 0x40U) printf(" SELF_CHECK");
        if (mfr & 0x08U) printf(" RESET_VOUT");
        if (mfr & 0x04U) printf(" BCX_FAULT");
        if (mfr & 0x02U) printf(" SYNC_FAULT");
        printf("\r\n");
    }
    if (have_cml && cml) {
        printf("[TPS] STATUS_CML=0x%02X:", (unsigned int)cml);
        if (cml & 0x80U) printf(" INVALID_COMMAND");
        if (cml & 0x40U) printf(" INVALID_DATA");
        if (cml & 0x20U) printf(" PEC_ERROR");
        if (cml & 0x10U) printf(" MEMORY_ERROR");
        if (cml & 0x08U) printf(" LOGIC_CORE_ERROR");
        if (cml & 0x02U) printf(" COMM_ERROR");
        printf("\r\n");
    }
    if ((status_word & 0x0040U) != 0U) {
        /* OFF bit set: the regulator shut down (fault response). Do NOT
           auto-retry - a repeated VOUT overvoltage on the ASIC core rail is
           a hardware-level risk. Re-enable consciously from the web UI. */
        printf("[TPS] WARNING: regulator is latched OFF after a fault - "
               "re-enable via web power switch after checking the cause\r\n");
    }
}
...
static void probe_i2c_devices(void) {
    if (!tps_detected) {
        ...
        if (tps_detected) {
            uint16_t vout_cmd_mv = 0;
            uint16_t status_word = 0;

#if TPS546_APPLY_BOARD_CONFIG
            /* The module NVM profile (648 mV, scale 0.5) does not match this
               board: it latches OFF with VOUT_OV + SYNC faults. Apply the
               Bitaxe-style BM1366 configuration before enabling the output. */
            {
                uint8_t cfg_step = tps546d24a_apply_board_config(
                                       &tps546d24a, BM1366_CORE_VOLTAGE_MV);
                if (cfg_step != 0U) {
                    uint8_t wp = 0, cml = 0;
                    printf("[I2C] TPS546D24A board config FAILED at step %u\r\n",
                           (unsigned int)cfg_step);
                    /* Report WRITE_PROTECT and the device's own complaint
                       for whichever PMBus write was rejected. */
                    if (tps546d24a_read_byte_cmd(&tps546d24a, 0x10U, &wp))
                        printf("[I2C] TPS546D24A WRITE_PROTECT=0x%02X "
                               "(0x80=all writes disabled except OPERATION)\r\n",
                               (unsigned int)wp);
                    if (tps546d24a_read_byte_cmd(&tps546d24a, 0x7EU, &cml) && cml)
                        printf("[I2C] TPS546D24A STATUS_CML=0x%02X "
                               "(bit7=invalid command, bit6=invalid data)\r\n",
                               (unsigned int)cml);
                } else {
                    printf("[I2C] TPS546D24A board config applied (%u mV)\r\n",
                           (unsigned int)BM1366_CORE_VOLTAGE_MV);
                }
            }
#endif
            if (!tps546d24a_set_enabled(&tps546d24a, tps_desired_enabled)) {
                printf("[I2C] TPS546D24A default power command failed\r\n");
            }
            /* The firmware never writes VOUT_COMMAND; report the module's
               own configured setpoint so the actual core voltage is visible.
               Also dump the OV/UV fault limits: an NVM configured for a
               different application (setpoint above the OV limit) produces
               exactly the "VOUT_OV fault -> latched OFF" signature. */
            if (tps546d24a_read_vout_command_mv(&tps546d24a, &vout_cmd_mv)) {
                uint16_t limit_mv = 0;
                uint16_t scale_loop_raw = 0;
                printf("[I2C] TPS546D24A VOUT_COMMAND=%u mV (module NVM setpoint)\r\n",
                       (unsigned int)vout_cmd_mv);
                if (tps546d24a_read_vout16_cmd_mv(&tps546d24a, 0x24U, &limit_mv))
                    printf("[I2C] TPS546D24A VOUT_MAX=%u mV\r\n", (unsigned int)limit_mv);
                if (tps546d24a_read_vout16_cmd_mv(&tps546d24a, 0x40U, &limit_mv)) {
                    uint32_t ov_actual_mv = (uint32_t)limit_mv * vout_cmd_mv / 1000U;
                    printf("[I2C] TPS546D24A VOUT_OV_FAULT_LIMIT=%.3f x VOUT_COMMAND (~%lu mV)%s\r\n",
                           (double)limit_mv / 1000.0,
                           (unsigned long)ov_actual_mv,
                           (vout_cmd_mv >= ov_actual_mv) ? "  <-- setpoint >= OV limit, OV fault guaranteed!" : "");
                }
                /* On this device the OV/UV/margin registers are ratios
                   (ULINEAR16) multiplied against VOUT_COMMAND. */
                if (tps546d24a_read_vout16_cmd_mv(&tps546d24a, 0x44U, &limit_mv))
                    printf("[I2C] TPS546D24A VOUT_UV_FAULT_LIMIT=%.3f x VOUT_COMMAND (~%lu mV)\r\n",
                           (double)limit_mv / 1000.0,
                           (unsigned long)((uint32_t)limit_mv * vout_cmd_mv / 1000U));
                if (tps546d24a_read_vout16_cmd_mv(&tps546d24a, 0x2BU, &limit_mv))
                    printf("[I2C] TPS546D24A VOUT_MIN=%u mV\r\n", (unsigned int)limit_mv);
                if (tps546d24a_read_vout16_cmd_mv(&tps546d24a, 0x25U, &limit_mv))
                    printf("[I2C] TPS546D24A VOUT_MARGIN_HIGH=%.3f x VOUT_COMMAND\r\n",
                           (double)limit_mv / 1000.0);
                if (tps546d24a_read_vout16_cmd_mv(&tps546d24a, 0x26U, &limit_mv))
                    printf("[I2C] TPS546D24A VOUT_MARGIN_LOW=%.3f x VOUT_COMMAND\r\n",
                           (double)limit_mv / 1000.0);
                if (tps546d24a_read_word_cmd(&tps546d24a, 0x29U, &scale_loop_raw))
                    printf("[I2C] TPS546D24A VOUT_SCALE_LOOP=0x%04X (SLINEAR11)\r\n",
                           (unsigned int)scale_loop_raw);
            }
            /* Latched fault bits survive forever without CLEAR_FAULTS; dump
               the detail registers once, then clear so STATUS_WORD reflects
               the live condition from here on. */
            if (tps546d24a_read_word_cmd(&tps546d24a, 0x79U, &status_word) &&
                status_word != 0U) {
                tps_report_fault_detail(status_word);
                tps546d24a_clear_faults(&tps546d24a);
            }
        }
    }
...
static void sample_i2c_telemetry(void) {
    ...
    if (tps_detected) {
        static uint16_t last_reported_tps_status = 0;
        ...
        if (tps_valid) {
            ...
            /* Report newly-latched faults (decode + clear) instead of letting
               stale bits persist in every telemetry frame forever. */
            if (tps_data.status_word != 0U &&
                tps_data.status_word != last_reported_tps_status) {
                tps_report_fault_detail(tps_data.status_word);
                tps546d24a_clear_faults(&tps546d24a);
            }
            last_reported_tps_status =
                (tps_data.status_word == 0U) ? 0U : tps_data.status_word;
...
static void check_bm1366_results(void) {
    ...
    static uint32_t last_hash_read_ms = 0;
    static uint32_t hash_counter_last = 0;
    static uint32_t hash_counter_last_ms = 0;
    uint32_t now = HAL_GetTick();

    /* Ask the ASIC for its total hash counter every 5s. The response is parsed
       by the same UART frame reader as nonce results, so this does not steal
       or corrupt nonce bytes. */
    if ((now - last_hash_read_ms) >= 5000U) {
        last_hash_read_ms = now;
        bm1366_send_hash_counter_read();
    }

    /* Route every valid 11-byte UART frame: bit 7 set = nonce result,
       otherwise a command/register response. */
    bm1366_result_raw_t raw;
    while (bm1366_read_any_result(&raw, 0) > 0) {
        if (raw.crc_and_flags & 0x80U) {
        ...
        /* Command response layout: value[2..5], asic_address[6], register[7]. */
        uint8_t *p = (uint8_t *)&raw;
        if (p[7] == 0x8CU) {   /* total hash counter */
            uint32_t counter = ((uint32_t)p[2] << 24) | ((uint32_t)p[3] << 16) |
                               ((uint32_t)p[4] << 8)  |  (uint32_t)p[5];
            uint32_t sample_ms = HAL_GetTick();
            if (hash_counter_last_ms != 0U && sample_ms > hash_counter_last_ms) {
                uint32_t dt_ms = sample_ms - hash_counter_last_ms;
                uint32_t delta = counter - hash_counter_last;
                if (dt_ms >= 1000U) {
                    /* Counter unit is 2^32 hashes (same convention as ESP-Miner). */
                    double ghs = (double)delta * 4294967296.0 /
                                 ((double)dt_ms * 1000000.0);
                    protocol_hashrate_t sample;
                    sample.hashrate_mhs = (uint32_t)(ghs * 1000.0);
                    uint8_t buf[16];
                    uint16_t len = protocol_encode_hashrate(&sample, buf);
                    if (len > 0) eth_send(buf, len);
                }
            }
            hash_counter_last = counter;
            hash_counter_last_ms = sample_ms;
        }
    }
}
        ...
        switch (type) {
            case MSG_JOB: {
                ...
                if (protocol_decode_job(payload, payload_len, &job) > 0) {
                    memcpy(&last_job, &job, sizeof(last_job));
                    last_job_valid = 1;
            ...
            case MSG_SET_PARAMS: {
                ...
                if (protocol_decode_setparams(payload, payload_len, &params) > 0) {
                    ...
                    apply_board_frequency(params.freq_mhz);
                    /* Voltage now actually applied (was ignored before).
                       tps546d24a_set_vout_mv enforces the 1000..2000 mV
                       window configured at boot. */
                    if (tps_detected &&
                        !tps546d24a_set_vout_mv(&tps546d24a,
                                                (uint16_t)params.voltage_mv)) {
                        printf("[PARAM] voltage %d mV rejected (range 1000-2000)\r\n",
                               params.voltage_mv);
                    }
                }
                break;
            ...
            case MSG_SET_FREQUENCY: {
                protocol_set_frequency_t freq;
                if (protocol_decode_set_frequency(payload, payload_len, &freq) > 0) {
                    apply_board_frequency(freq.frequency_mhz);
                }
                break;
            }
