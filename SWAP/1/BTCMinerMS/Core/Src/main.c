...
#define BM1366_EXPECTED_COUNT  2    /* number of BM1366 chips in the UART chain; set to 1 on a single-chip bench board */
#define BM1366_TARGET_FREQ_MHZ 200.0f  /* default ASIC working frequency at boot */
/* TPS546D24A has been removed from this board. The ASIC core rail is now
   supplied by an always-on external power stage controlled by the operator,
   so no firmware power-on or PMBus telemetry path is required. Keep the
   driver source in the build but compile out every call site. */
#define TPS546_PRESENT             0
#if TPS546_PRESENT
...
#else
#define TPS546_APPLY_BOARD_CONFIG  0
#define BM1366_CORE_VOLTAGE_MV     0U
#endif
...
int main(void)
{
  ...
  while (1) {
    ...
    /* TPS/sensor detection poll: every 10 s in every state, so a
       slow-starting or hot-plugged power module is (re)discovered and
       configured within 10 s - not only when nothing was detected. */
    if ((now - last_sensor_probe) >= 10000U) {
...
static void probe_i2c_devices(void) {
    ...
    if (!tps_detected) {
        ...
        if (tps_detected) {
            ...
            {
                uint16_t vin_mv = 0;
                if (tps546d24a_read_vin_mv(&tps546d24a, &vin_mv)) {
                    printf("[I2C] TPS546D24A VIN=%u mV%s\r\n",
                           (unsigned int)vin_mv,
                           (vin_mv < 11000U)
                               ? "  <-- below the disabled 12V VIN_ON profile"
                               : "");
                } else {
                    printf("[I2C] TPS546D24A READ_VIN failed\r\n");
                }
            }
            /* Dump-and-clear any fault latched during power-up so
               STATUS_WORD reflects the live condition from here on. */
            {
                uint16_t status_word = 0;
                if (tps546d24a_read_word_cmd(&tps546d24a, 0x79U, &status_word) &&
                    status_word != 0U) {
                    printf("[I2C] TPS546D24A power-up STATUS_WORD=0x%04X (cleared)\r\n",
...
static void sample_i2c_telemetry(void) {
    ...
    if (tps_detected) {
        ...
        if (tps_valid) {
            ...
            /* Report a newly-latched fault once (then clear) instead of
               letting stale bits persist in every telemetry frame. */
            if (tps_data.status_word != 0U &&
                tps_data.status_word != last_reported_tps_status) {
                printf("[TPS] fault: STATUS_WORD=0x%04X READ_VOUT=%u mV IOUT=%lu mA\r\n",
                       (unsigned int)tps_data.status_word,
                       (unsigned int)tps_data.vout_mv,
                       (unsigned long)tps_data.iout_ma);
                tps546d24a_clear_faults(&tps546d24a);
            }
    ...
    while (off + 4 <= net_rx_buffered) {
        ...
        switch (type) {
            ...
            case MSG_SET_POWER: {
                ...
                (void)payload;
                (void)payload_len;
                reply_len = protocol_encode_error(0x21,
                    "TPS546D24A removed; ASIC rail is externally controlled", reply);
#endif
