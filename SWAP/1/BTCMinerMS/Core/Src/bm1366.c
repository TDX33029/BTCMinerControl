...
/* Discard every byte already captured by the USART/ring buffer and reset the
   signal-integrity counters. This is called before a fresh chip-ID probe so
   reset/power-up noise cannot masquerade as an ASIC frame. While the RXNE
   interrupt is briefly disabled we also execute the F1 "read SR then DR"
   sequence, which clears ORE/FE/NE; an ORE left over from the period before
   bm1366_uart_init() would otherwise drop the first real response byte. */
static void bm1366_uart_rx_reset(void) {
    uint8_t dummy;
...
/* CubeMX configures PA10 as a floating input. While the ASIC TX pin is
   tri-state during RESET/power-up, that floating RX can couple from the
   adjacent 50 MHz MCO track and produce spurious 0xAA UART frames
   ("blocked on AA"). Pull the idle level high until the ASIC drives it. */
static void bm1366_uart_apply_rx_pullup(void) {
    GPIO_InitTypeDef rx_pin;

    __HAL_RCC_GPIOA_CLK_ENABLE();
    rx_pin.Pin   = GPIO_PIN_10;
    rx_pin.Mode  = GPIO_MODE_INPUT;
    rx_pin.Pull  = GPIO_PULLUP;
    rx_pin.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(GPIOA, &rx_pin);
}

/* Passive continuity test between PA9 (TX) and PA10 (RX), run while the ASIC
   is held in RESET so its pins do not drive the nets. PA9 becomes a pulled-up
   input and PA10 a push-pull output; if PA9 follows both PA10 levels, there
   is a conductive RX/TX loopback on the board (jumper, solder bridge, or an
   intentionally single-wired UART). This is the test that distinguishes
   "chip echoes commands" from "MCU is hearing its own transmitter". */
static uint8_t bm1366_uart_rxtx_short_test(void) {
    GPIO_InitTypeDef gpio;
    uint8_t pa9_low, pa9_high;
    uint8_t shorted;
    ...
    HAL_Delay(1);
    pa9_low = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_9);

    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_10, GPIO_PIN_SET);
    HAL_Delay(1);
    pa9_high = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_9);

    shorted = (pa9_low == GPIO_PIN_RESET && pa9_high == GPIO_PIN_SET) ? 1U : 0U;

    /* Restore USART1 pin functions, the PA10 pull-up and a clean receiver. */
    HAL_UART_MspInit(&huart1);
    bm1366_uart_apply_rx_pullup();
    __HAL_UART_ENABLE(&huart1);
    bm1366_uart_rx_reset();
    HAL_NVIC_EnableIRQ(USART1_IRQn);
    return shorted;
}

void bm1366_uart_init(void) {
    /* Pull RX high until the ASIC drives it, clear any stale DR/RXNE/ORE/FE
       bytes captured while the receiver ran without an ISR, then enable the
       RXNE interrupt for the BM1366 protocol handler. */
    bm1366_uart_apply_rx_pullup();
    bm1366_uart_rx_reset();
    printf("[CHIP] USART1 CR1=0x%08lX CR3=0x%08lX BRR=0x%08lX "
           "(HDSEL must be 0)\r\n",
           (unsigned long)USART1->CR1, (unsigned long)USART1->CR3,
...
int bm1366_read_any_result(bm1366_result_raw_t *result, uint32_t timeout_ms) {
 ...
 static void bm1366_print_uart_gpio(const char *tag) {
    printf("[CHIP] %s GPIO: RST(PB0)=%d RX(PA10)=%d TX(PA9)=%d\r\n",
           tag,
           (int)HAL_GPIO_ReadPin(BM1366_RST_PORT, BM1366_RST_PIN),
           (int)HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_10),
           (int)HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_9));
}

 int bm1366_count_chips(uint8_t expected_count) {
     /* init3 already sent the chip-ID read -- do NOT re-send (would double the
        responses). Drain the 11-byte replies with a rolling window so a stray
        0xAA from reset/power-up noise can NEVER swallow a real response that
        arrives just behind it (the old parser waited up to 1000 ms after a
        stray AA and discarded everything it consumed, which is exactly the
        intermittent "blocked on AA" symptom).
        Response layout (matches ESP-Miner count_asic_chips):
     ...
     uint16_t found = 0;
     uint32_t start = HAL_GetTick();
     /* Rolling window keeps the last received bytes; a candidate frame is
        checked every time the window grows, regardless of preceding noise. */
     uint8_t window[32];
     ...
     while ((HAL_GetTick() - start) < total_wait && found < expected_count) {
         ...
         if (window_len < (uint16_t)(sizeof(window) / sizeof(window[0]))) {
             window[window_len++] = byte;
         } else {
             uint16_t i;
             for (i = 0; i + 1U < (uint16_t)(sizeof(window) / sizeof(window[0])); i++) {
                 window[i] = window[i + 1U];
             }
             window[(sizeof(window) / sizeof(window[0])) - 1U] = byte;
         }

         /* The 11-byte ID response, if it ends at this byte, is the last 11
            bytes of the window. Check it independent of any earlier AA noise. */
         if (window_len >= 11U) {
             const uint8_t *cand = &window[window_len - 11U];
             uint16_t chip_id;

             if (cand[0] != 0xAAU || cand[1] != 0x55U) continue;
             chip_id = ((uint16_t)cand[2] << 8) | cand[3];
             if (chip_id != BM1366_CHIP_ID) continue;
             if (bm1366_crc5(cand + 2, 9) != 0) {
                 rx_bad_crc++;
                 continue;
             }
             found++;
             window_len = 0U;   /* frame consumed: start collecting the next one */
         }
     }
     ...
     printf("\r\n[CHIP] found=%u  total=%lu  FE/NE=%u ORE=%u bad_crc=%lu\r\n",
            (unsigned)found, (unsigned long)uart_rx_total,
            (unsigned)uart_rx_frame_errors, (unsigned)uart_rx_overruns,
            (unsigned long)rx_bad_crc);
     return found;
 }
 int bm1366_init_chips(uint8_t expected_count, float target_freq_mhz) {
     ...
     /* Discard any bytes captured before the reset pulse (power-up / previous
        run noise). The diagnostic below must only show post-command traffic. */
     bm1366_uart_rx_reset();
     bm1366_print_uart_gpio("before reset");
     ...
     printf("[CHIP] reset pulse (RST low 100ms -> high 250ms)\r\n");
     HAL_GPIO_WritePin(BM1366_RST_PORT, BM1366_RST_PIN, GPIO_PIN_RESET);
     HAL_Delay(10);
     printf("[CHIP] RX/TX short test: %s\r\n",
            bm1366_uart_rxtx_short_test() ? "SHORTED (loopback path exists)"
                                           : "OPEN (separate TX/RX nets)");
     HAL_Delay(90);
     HAL_GPIO_WritePin(BM1366_RST_PORT, BM1366_RST_PIN, GPIO_PIN_SET);
     HAL_Delay(250);
     bm1366_print_uart_gpio("after reset");

     /* Drop the burst a chip can emit while coming out of RESET; it is not a
        response to the ID command and looks like a stream of 0xAA bytes. */
     bm1366_uart_rx_reset();
     ...
     /* Read chip ID */
     const uint8_t init3[] = {0x55, 0xAA, 0x52, 0x05, 0x00, 0x00, 0x0A};
     bm1366_send_raw(init3, 7);
 
     int chips = bm1366_count_chips(expected_count);

     /* Some BM1366 boards are not UART-ready at the first 100-300 ms mark
        after RESET and silently miss the one-shot ID command. Give the chip
        a completely fresh reset + ID read before declaring the chain empty. */
     if (chips == 0) {
         printf("[CHIP] first ID read timed out; resetting again and retrying\r\n");
         HAL_GPIO_WritePin(BM1366_RST_PORT, BM1366_RST_PIN, GPIO_PIN_RESET);
         HAL_Delay(100);
         HAL_GPIO_WritePin(BM1366_RST_PORT, BM1366_RST_PIN, GPIO_PIN_SET);
         HAL_Delay(250);
         bm1366_print_uart_gpio("retry after reset");
         bm1366_uart_rx_reset();
         for (int i = 0; i < 3; i++) {
             bm1366_set_version_mask(0x1FFFE000);
         }
         bm1366_send_raw(init3, 7);
         chips = bm1366_count_chips(expected_count);
     }
