...
void eth_rx_flush(void);  /* discard stale TCP RX ring bytes on disconnect */
int  eth_is_connected(void);
int  eth_has_ip(void);       /* true once DHCP has bound an IPv4 address */
void eth_get_ip(uint8_t out[4]); /* current IPv4 address (0.0.0.0 if unbound) */
void eth_poll(void);
