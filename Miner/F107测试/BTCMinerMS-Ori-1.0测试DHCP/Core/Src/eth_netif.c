...
static void    dump_dhcp_if_any(const uint8_t *buf, uint16_t len);
...
static err_t low_level_output(struct netif *netif, struct pbuf *p) {
    ...
    dump_dhcp_if_any(dst, (uint16_t)offset);   /* DIAG: dump TX DHCP (DISCOVER/REQUEST) */
...
/* DIAG: if buf is a DHCP packet (IPv4/UDP ports 67/68), print key fields so we
   can see whether OFFER/ACK arrive and whether they're addressed to this board. */
static void dump_dhcp_if_any(const uint8_t *buf, uint16_t len) {
    if (len < 14 + 20 + 8 + 44) return;            /* ETH + IP + UDP + DHCP hdr */
    uint16_t ethertype = ((uint16_t)buf[12] << 8) | buf[13];
    if (ethertype != 0x0800) return;                /* not IPv4 */
    uint8_t ihl = (buf[14] & 0x0F) * 4;
    if (ihl < 20 || (uint32_t)(14 + ihl + 8) > len) return;
    if (buf[14 + 9] != 17) return;                  /* not UDP */
    uint32_t udp_off = 14 + ihl;
    uint16_t sport = ((uint16_t)buf[udp_off] << 8) | buf[udp_off + 1];
    uint16_t dport = ((uint16_t)buf[udp_off + 2] << 8) | buf[udp_off + 3];
    if (sport != 67 && sport != 68 && dport != 67 && dport != 68) return;
    uint32_t doff = udp_off + 8;
    if (doff + 44 > len) return;
    uint8_t op = buf[doff + 0];
    uint32_t xid = ((uint32_t)buf[doff + 4] << 24) | ((uint32_t)buf[doff + 5] << 16)
                 | ((uint32_t)buf[doff + 6] << 8) | buf[doff + 7];
    const uint8_t *yi = &buf[doff + 16];            /* yiaddr (offered IP) */
    const uint8_t *ch = &buf[doff + 28];            /* chaddr (client MAC) */
    printf("[DHCP] %s xid=0x%08lX yi=%d.%d.%d.%d ch=%02X:%02X:%02X:%02X:%02X:%02X sp=%lu dp=%lu\r\n",
           (op == 1) ? "REQ" : (op == 2) ? "REPLY" : "?",
           (unsigned long)xid,
           yi[0], yi[1], yi[2], yi[3],
...
static struct pbuf *low_level_input(void) {
    ...
    for (i = 0; i < (int)rx_count; i++) {
        ...
        dump_dhcp_if_any(eth_rx_buff[i], len);

        /* DIAG: hex dump first 60 bytes of large frames to identify them
           (is a 322B frame a DHCP OFFER, VLAN-tagged, or something else?). */
        if (len > 300) {
            printf("[HEX] %uB:", (unsigned)len);
            for (uint16_t k = 0; k < 60 && k < len; k++) printf(" %02X", eth_rx_buff[i][k]);
            printf("\r\n");
        }
...
void ethernetif_input(struct netif *netif) {
    ...
    while ((p = low_level_input()) != NULL) {
        /* Feed to LwIP */
        err_t e = netif->input(p, netif);
        if (e != ERR_OK) {
            printf("[NETIF] input dropped: err=%d\r\n", (int)e);
            pbuf_free(p);
        }
    }
}
