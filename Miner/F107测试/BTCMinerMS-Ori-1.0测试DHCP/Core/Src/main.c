...
/* IPv4 address/gateway/netmask are obtained via DHCP at runtime
...
int main(void)
{
  ...
  /* Original board: 50MHz RMII reference clock comes from an external crystal
     shared by the MAC (PA1 REF_CLK) and the DP83848 -- no MCO/PLL3 to check. */
  printf("[CLK] HSE 24MHz, 50MHz RMII ref from external crystal\r\n");
...
void SystemClock_Config(void)
{
  ...
  /* Original board: HSE 24MHz -> PREDIV1(/3) = 8MHz -> PLL(x9) = 72MHz.
     No PLL2/PLL3/MCO -- the 50MHz RMII reference clock comes from an external
     crystal shared by the MAC (PA1 REF_CLK) and the DP83848. */
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin : PA8 (unused on original board -- 50MHz ref is external) */
