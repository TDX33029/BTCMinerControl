...
#define PHY_ADDR    0x00
 ...
 static int phy_init(void) {
    /* DP83848 hardware reset via PD2 */
    {
        GPIO_InitTypeDef g;
        ...
        g.GPIO_Speed = GPIO_Speed_50MHz;
        GPIO_Init(GPIOD, &g);
        GPIO_ResetBits(GPIOD, GPIO_Pin_2);    /* hold reset low */
        Delay_ms(10);
        GPIO_SetBits(GPIOD, GPIO_Pin_2);      /* release reset */
        Delay_ms(100);                          /* wait for PHY ready */
    }
    ...
    if (id1 == 0xFFFF || id1 == 0x0000) { printf("[PHY] ID1 invalid, PHY not responding\r\n"); return 0; }
    if (id2 == 0xFFFF || id2 == 0x0000) { printf("[PHY] ID2 invalid\r\n"); return 0; }
 
    /* Skip software reset - PHY was HW-reset via NRST. */
    /* Start auto-negotiation directly. */
    printf("[PHY] Auto-negotiate...\r\n");
    ETH_WritePHYRegister(PHY_ADDR, 0x00, 0x1200);
    uint32_t t = 0xFFFFFF;
    while (!(ETH_ReadPHYRegister(PHY_ADDR, 0x01) & 0x0020)) {
        if (!--t) { printf("[PHY] AN timeout\r\n"); return 0; }
    }
