/**
 * BM1366 ASIC driver - STM32F103C8T6 / STM32F107VCT6
 * USART1 based communication with multi-chip daisy chain
 */
#include "bm1366.h"
#include "Delay.h"
#include <string.h>
...
/* ===== Constants ===== */
#define BM1366_DEFAULT_BAUD  115749
...
void bm1366_uart_init(void) {
    ...
    nvic.NVIC_IRQChannel = USART1_IRQn; nvic.NVIC_IRQChannelPreemptionPriority = 1;
    nvic.NVIC_IRQChannelSubPriority = 0; nvic.NVIC_IRQChannelCmd = ENABLE;
    NVIC_Init(&nvic); USART_Cmd(USART1, ENABLE);
...
/* ===== Read Result with CRC5 verification ===== */
...
static void bm1366_get_difficulty_mask(uint16_t difficulty, uint8_t mask[6]) {
    mask[0] = 0x00; /* 芯片地址 = 0x00 (广播) */
    mask[1] = 0x14; /* 寄存器 0x14 = 难度 */
    mask[2] = 0x00;
    mask[3] = 0x00;
...
void bm1366_set_version_mask(uint32_t mask) {
    ...
    uint8_t cmd[] = {
        ...
        0x90,           /* ? */
        0x00,
        (uint8_t)(versions_to_roll >> 8),
        (uint8_t)(versions_to_roll & 0xFF)
    };
    bm1366_send_cmd(BM1366_TYPE_CMD | BM1366_GROUP_ALL | BM1366_CMD_WRITE,
                    cmd, 6);
}
...
static void bm1366_set_chip_address(uint8_t chip_addr) {
    ...
    bm1366_send_cmd(BM1366_TYPE_CMD | BM1366_GROUP_SINGLE | BM1366_CMD_SETADDRESS,
                    data, 2);
}
...
static void bm1366_send_chain_inactive(void) {
    ...
    bm1366_send_cmd(BM1366_TYPE_CMD | BM1366_GROUP_ALL | BM1366_CMD_INACTIVE,
                    data, 2);
}
...
float bm1366_set_frequency(float target_freq_mhz) {
    ...
    /* 电压档位: fb*MULT/ref >= 2400 → 0x50, 否则 0x40 */
...
int bm1366_count_chips(uint8_t expected_count) {
    ...
    bm1366_send_cmd(BM1366_TYPE_CMD | BM1366_GROUP_ALL | BM1366_CMD_READ,
                    cmd, 2);

    /* 等待响应 — 每颗芯片会返回 11 字节 ID 响应 */
    uint16_t total_wait = 100; /* 最多等 100ms */
    uint16_t found = 0;
    extern volatile uint32_t g_ms;
    uint32_t start = g_ms;
...
int bm1366_init_chips(uint8_t expected_count, float target_freq_mhz) {
    ...
    gpio.GPIO_Pin   = BM1366_RST_PIN;
    gpio.GPIO_Mode  = GPIO_Mode_Out_PP;
    gpio.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(BM1366_RST_PORT, &gpio);
    GPIO_SetBits(BM1366_RST_PORT, BM1366_RST_PIN);

    /* ===== BM1366 初始化序列 (来自 ESP-Miner bm1366.c: BM1366_init) ===== */

    /* 设置 version mask (发 3 次，确保可靠) */
    for (int i = 0; i < 3; i++) {
        bm1366_set_version_mask(0x1FFFE000); /* STRATUM_DEFAULT_VERSION_MASK */
    }

    /* 读取芯片 ID — 使用简单指令格式 [0x55][0xAA][0x52][0x05][0x00][0x00][0x0A] */
    {
...
uint8_t bm1366_get_address_interval(void) {
    return bm1366_address_interval;
}

uint8_t bm1366_get_chip_count(void) {
    return bm1366_chip_count;
}


/* ===== Frequency transition (safe PLL ramping) ===== */
#define FREQ_STEP_SIZE  6.25f   /* MHz per step, matching ESP-Miner */
#define FREQ_EPSILON    0.0001f

void bm1366_frequency_transition(float target_freq_mhz, uint32_t step_delay_ms) {
    /* Read current actual frequency from last set call */
    /* Since we don't track current freq statically, compute from target */
    ...
    /* If within one step, set directly */
    if (diff < FREQ_STEP_SIZE) {
        bm1366_set_frequency(target_freq_mhz);
        return;
    }
    
    /* Ramp in FREQ_STEP_SIZE increments */
...
/*
 * Set core voltage scale (vdo_scale register).
 * vdo_scale: 0x40 for normal, 0x50 for high voltage
 */
