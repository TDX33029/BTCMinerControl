...
更换外置晶振为 25MHz 后, 需在 CubeMX 中重新配置 RCC:
- 调整 PLL2 / PREDIV / PLL 分频比以保持 SYSCLK=72MHz
- 示例: HSE(25MHz) → PREDIV2(÷5) → 5MHz → PLL2(x9) → 45MHz → PREDIV1(÷5) → 9MHz → PLL(x8) → 72MHz
- 72MHz 下 USART1 BRR 使用 HAL_RCC_GetPCLK2Freq() 已自动适应
- 重新生成后检查 MX_GPIO_Init 对 PC13/PB1/PB14/LED 等 GPIO 的初始化代码

# ⚠ 时钟切换备忘录 (24MHz → 25MHz HSE)